 ...
 */
static void
create_padded_key(uint8_t *outkey, uint8_t *inkey, uint16_t inkeylen)
{
 ...
 */
static void
xor_array(uint8_t *out, uint8_t *ina, uint8_t *inb, uint16_t len)
{
 ...
 */
static void
merge_arrays(uint8_t *out, uint8_t *ina, uint16_t lena, uint8_t *inb, uint16_t lenb)
{
 ...
 */
static void
hash_sha1(uint8_t *out, uint8_t *in, uint16_t inlen)
{
...
void
PRINT_BUF(const char *name, unsigned char *buf, unsigned int size)
{
  int i = 0;
  dy_printf("BUF:%s\n", name);
  for(; i < size; i++) {
...
void
hmac_sha1(hmac_data_t *hmac_data)
{
