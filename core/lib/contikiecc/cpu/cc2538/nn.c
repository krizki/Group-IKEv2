/**
 * \defgroup cc2538 CC2538 specific implementation
 * \ingroup nn
 * @{
 */
...
NN_Add(NN_DIGIT *a, NN_DIGIT *b, NN_DIGIT *c, NN_UINT digits)
{
  ...
  carry = 0;
#ifdef HW_ECC
  /* Variables used in the addition  */
  ...
  /* Start the addition and wait for the operation to finish*/
  pka_status =  PKABigNumAddStart(b,digits,c,digits,&resultVector);
  do {} while(PKAGetOpsStatus() == PKA_STATUS_OPERATION_INPRG);
  
  if(pka_status == PKA_STATUS_SUCCESS){
    /* Wait for the PKA driver to become available */
    pka_status = PKABigNumAddGetResult(a,&pui32Len,resultVector);
    
    if (pka_status == PKA_STATUS_SUCCESS){
      return carry;
    } else if(pka_status == PKA_STATUS_BUF_UNDERFLOW){
      /* The a buffer was not large enough for the result (happens infrequently)*/
      uint8_t newlen;
      newlen = digits + 1;
      resultVector = 0;
      pui32Len = newlen;
      ...
      PKABigNumAddStart(b,digits,c,digits,&resultVector);
      
      /* Wait for the operation to finish */
      do {} while(PKAGetOpsStatus() == PKA_STATUS_OPERATION_INPRG);
      pka_status = PKABigNumAddGetResult(z,&pui32Len,resultVector);
      if(pka_status == PKA_STATUS_SUCCESS){
        /* Copy the result to the a buffer */
        memcpy(a,&z,digits*NN_DIGIT_LEN);
        carry = 1;
        return carry;
      }
    }
...
NN_Mult(NN_DIGIT *a, NN_DIGIT *b, NN_DIGIT *c, NN_UINT digits)
{
  ...
  pka_status =  PKABigNumMultiplyStart(b,digits,c,digits,&resultVector);
  do {} while(PKAGetOpsStatus() == PKA_STATUS_OPERATION_INPRG);
  if(pka_status == PKA_STATUS_SUCCESS){
    /* Assuming that a is always 2 * digits */
    pka_status = PKABigNumMultGetResult(a,&pui32Len,resultVector);
  } 
#else
...
{ 
  ...
  /* Wait for the PKA driver to become available */
  do {} while(PKAGetOpsStatus() == PKA_STATUS_OPERATION_INPRG);
  pka_status =  PKABigNumModStart(b,b_digits,c,c_digits,&resultVector);
  
  do {} while(PKAGetOpsStatus() == PKA_STATUS_OPERATION_INPRG);
  if(pka_status == PKA_STATUS_SUCCESS){
    /* Assuming that a is always 2 * digits */
    pka_status = PKABigNumModGetResult(a,ui8NLen,resultVector);
  }
#else
...
NN_ModInv(NN_DIGIT *a, NN_DIGIT *b, NN_DIGIT *c, NN_UINT digits)
{
  ...
  pka_status =  PKABigNumInvModStart(b,digits,c,digits,&resultVector);
 
  do {} while(PKAGetOpsStatus() == PKA_STATUS_OPERATION_INPRG);
  if(pka_status == PKA_STATUS_SUCCESS){
    /* Assuming that a is always digits */
    pka_status = PKABigNumInvModGetResult(a,ui8NLen,resultVector);
  }
...
NN_Cmp(NN_DIGIT *a, NN_DIGIT *b, NN_UINT digits)
{
  ...
  do {} while(PKAGetOpsStatus() == PKA_STATUS_OPERATION_INPRG);
  pka_status =  PKABigNumCmpStart(a,b,digits);

  /* Wait for the PKA driver to finish the operation */
  do {} while(PKAGetOpsStatus() == PKA_STATUS_OPERATION_INPRG);
  if(pka_status == PKA_STATUS_SUCCESS){
    pka_status = PKABigNumCmpGetResult();
